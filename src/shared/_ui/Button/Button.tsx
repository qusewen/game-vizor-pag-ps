...
export const Button = ({ title, variant = 'default', onClick, style, classNames, children, disabled = false }: IProps) => {
  ...
  return (
    <button
      ...
      id={`${disabled}`}
      disabled={disabled}
      style={{ borderRadius: '10px', ...style, borderWidth: '1px', borderColor: variantStyles.border, borderStyle: 'solid', backgroundColor: !disabled ? `${variantStyles?.backgroundColor}` : ""  }}
      className={clsx(
